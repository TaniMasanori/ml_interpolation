#!/usr/bin/env python3
"""
This script runs SPECFEM3D simulation steps for testing.
"""
import os
import sys
import numpy as np
...
output_dir = os.path.join(project_root, "data/synthetic/raw/simulation_test")
...
runner = SpecfemSimulation(specfem_dir, output_dir)

# Copy the Par_file template to the expected location
par_file_template = os.path.join(project_root, "specfem_simulations/Par_file_template")
with open(par_file_template, 'w') as f:
    f.write("""#-----------------------------------------------------------
#
...
""")
print(f"Created Par_file template at {par_file_template}")
...
if success:
    ...
    # Check for seismogram files
    seismo_files = [f for f in os.listdir(output_dir) if f.endswith(".semd")]
    if seismo_files:
        print(f"Found {len(seismo_files)} seismogram files in {output_dir}")
        # Show a few file names as examples
        for file in seismo_files[:5]:
            print(f"  - {file}")
        if len(seismo_files) > 5:
            print(f"  - ... and {len(seismo_files) - 5} more files")
    else:
        print(f"\n❌ No seismogram files found in {output_dir}")
else:
